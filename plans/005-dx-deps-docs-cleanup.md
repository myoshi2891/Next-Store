...
このステップの時点で `bun run test`（通常の Vitest 探索）が対象とするのは
モック済みテストのみである（DB・Clerk・Stripe の実キーは不要な構成にする。
もしテストが環境変数を要求して落ちる場合はダミー値を env に設定するのではなく
STOP して報告する — テスト側の分離不足が真因のため）。

**plans/004-data-integrity-and-performance.md との整合**: 004 の Step 4 は実際の
PostgreSQL に接続する統合テスト（`addToCartAction` の並行実行で Serializable の
直列化失敗 `P2034` や `Cart.version` 楽観的ロック衝突を検証する）を、004 自身の
記載どおり「別ファイル・別スクリプトへの分離」を前提に追加する計画である。本プラン
（005）の実行時点ではその統合テストファイルはまだ存在しないため、`vitest.config.ts`
の除外設定や統合テスト専用スクリプトの追加は本プランのスコープ外（`package.json` の
編集は scripts に限られる — 上記 Scope 参照）とし、004 側で統合テストファイルを
追加する際に併せて行う。ただし本 Step 7 の CI ワークフローは、その将来の追加を
妨げない・かつ誤って「モックテストの前提」を壊さない構成にすること:
- 手順 7 の `bun run test` ジョブに実キー・DB 接続を要求しない、という上記の
  STOP 条件は「現時点で存在するテストファイル」にのみ適用される。004 が
  DB 接続前提の統合テストファイル（別パターン・別スクリプトに分離されたもの）を
  追加した後は、その統合テストは本ジョブの通常実行対象から除外され、PostgreSQL
  サービスコンテナと `DATABASE_URL` を提供する別ジョブでのみ実行される想定である
  ことを、このプランの完了報告（Maintenance notes 相当）に明記する。
- 本プランの CI ワークフローの手順 7 を、後から統合テスト用ジョブを追加しやすい
  独立したジョブ構成（例: 1 ワークフローファイル内の複数 `jobs:` エントリ）で
  書くこと。単一ジョブにすべての手順を詰め込み、後から DB 起動ステップを
  無理に割り込ませる必要がある構成にはしない。
