...
- `imageSchema`（`validateImageFile` への委譲を網羅）。Plan 001 で `imageSchema` は
  ...
  1つずつ用意する）:
  - サイズ: `utils/schemas.ts:55` の `maxUploadSize = 1024 * 1024`（= 1,048,576
    バイト）と `:60` の `file.size <= maxUploadSize`（境界値を含む）に合わせ、
    上記の有効フィクスチャの末尾（PNG の IEND チャンク後、JPEG の EOI マーカー
    `0xFFD9` 後、GIF のトレーラ `0x3B` 後 — デコーダが無視する末尾領域）に
    パディングバイトを追加して次の3ケースを作る（WebP は RIFF チャンクサイズ
    フィールドと整合しないパディングだとデコードに失敗し得るため、末尾パディング
    ではなく余白用チャンクの追加、またはこの形式はサイズ境界ケースの対象から外し
    許可 MIME ケースの最小フィクスチャのみとする）:
    - ちょうど 1,048,575 バイト → 成功（`<` の境界）
    - ちょうど 1,048,576 バイト → 成功（`<=` により境界値ぴったりも許可される）
    - ちょうど 1,048,577 バイト → 失敗（初めて上限を超える）
  ...
  - **MIME スプーフィング**: `type: "image/jpeg"` と宣言しつつ実際のバイト列は
    ...
    Plan 001 のバイト署名検証が適用されているため、このケースは**拒否される**
    ことを期待値として記録する。
...
- [ ] allowlist（Scope の 5 ファイル `__tests__/utils/cart-calculations.test.ts`,
      `__tests__/utils/order-actions.test.ts`, `__tests__/api/payment-route.test.ts`,
      `__tests__/api/confirm-route.test.ts`, `__tests__/utils/schemas.test.ts`、
      および Step 1 で作成した場合のみ `__tests__/helpers/mock-db.ts`）に対し、
      開始時・完了時それぞれで以下 4 コマンドを `--name-only` で実行して変更ファイルを
      収集し、allowlist 外のパスが 1 件でも含まれていれば失敗とする
      （`__tests__/` 配下というディレクトリ単位の広い許可ではなく、この明示的な
      ファイル一覧との突き合わせで判定する）:
      `git diff --name-only 90f91f4..HEAD`、`git diff --cached --name-only`、
