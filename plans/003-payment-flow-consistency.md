...
4. **注文が古い合計値から作られ得る**: `createOrderAction` は保存済みの cart 合計を
   信頼し、作成時に再計算しない。
5. **ドキュメントと実装の価格単位が不一致**: CLAUDE.md は「セント単位で保存」と
...
**Out of scope**:
- `prisma/schema.prisma` — 型変更（Int→Float 等）はしない。丸めで整数を維持する。
...
- `fetchOrCreateCart` の後に `const { currentCart } = await updateCart(cart);` を
  呼び、`currentCart` の値から Order を作成する（保存済み値への依存を排除）。
...
Plan 002 の `payment-route.test.ts` の期待値を「line_items 合計 = orderTotal × 100」に更新。
...
未完了セッションでは order 更新も cart 削除も行われない。
...
- 追加ケース: line_items 合計と orderTotal の一致アサーション（Step 3）、
  confirm の冪等性（同一 session_id で 2 回呼んでもエラーにならない）。
