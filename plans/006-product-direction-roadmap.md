...
> 変更を報告したら、`plans/README.md` とそれ以外の対象パスを排他的に扱わず、
> 検出された各パスへ個別に適用する: `plans/README.md` に差分があれば当該表の
> 選択・状態変更を確認し、`app/` や `package.json` など他の対象パスにも同時に
> 差分があれば各項目の Evidence も併せて再確認する。両方に差分がある場合は
> 両方を確認し、内容が食い違えば停止すること。
